...
We've created an interactive webpage, focusing on the UI to optimise the experience for our users. With a maximum of 6 inputs - it is not required to fill each one, a user will receive a detailed report on how their data affects the likelihood of developing diabetes. 
...
However, only the Logistic Regression model contains the feature we need, the "weights" feature. This feature shows the direct influence between an input and the model's output. We used these "weights" to precisely identify how each parameter would affect the development of diabetes. 
...
This allowed us to write the logic into JavaScript code, a simple implementation onto a website. Each form filled from a user is run through algorithms using the array of weights to create an interpretable value for the influence of each, individual feature.
